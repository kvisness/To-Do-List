...
// Get all tasks
router.get('/', (req, res) => {
  let queryText = `SELECT * FROM "tasklist" ORDER BY "due-date";`;
...
router.post('/', (req, res) => {
  ...
  let queryText = `INSERT INTO "tasklist" ("request", "status", "priority", "due-date", "notes") VALUES ($1, $2, $3, $4, $5);`;
  pool.query(queryText, [newTask.request, newTask.status, newTask.priority, newTask.due-date, newTask.notes])
    .then((result) => {
